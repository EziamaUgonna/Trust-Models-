...
def pearson_correlation(opinions, recommendations):
    corr, _ = pearsonr(opinions, recommendations)
    return corr
...
def compute_trust(correlation, entropy_similarity, entropy_recommendation, prev_trust, theta=0.5):
...
# Compute similarity (using Pearson correlation) and entropy for each recommender
similarities = []
entropies_similarity = []
entropies_recommendation = []

for rec in recommendations:
    corr = pearson_correlation(E_opinions, rec)
    similarities.append(corr)
    entropies_similarity.append(entropy(corr))
...
print("Similarity Scores (Pearson Correlation):", similarities)
